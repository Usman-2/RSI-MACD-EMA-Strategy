...
def execute_orders(predictions, open_prices, current_position, signals_df):
    orders = []
    closed_signals = []
    ...
    for i in range(len(predictions)):
        if predictions[i] == 1:  # Buy signal
            if current_position == 'none':
                ...
                signals_df = pd.concat([signals_df, pd.DataFrame([{'Date': open_prices.index[i], 'Price': open_prices.iloc[i], 'Signal': 'Buy', 'Closed': 'No'}])], ignore_index=True)
            ...
            if current_position == 'long':
                ...
                closed_signals.append(('Closed', open_prices.iloc[i], current_position))
                signals_df = pd.concat([signals_df, pd.DataFrame([{'Date': open_prices.index[i], 'Price': open_prices.iloc[i], 'Signal': 'Sell', 'Closed': 'Yes'}])], ignore_index=True)
                current_position = 'none'
            elif current_position == 'none':
                orders.append(('Sell', open_prices.iloc[i]))
                signals_df = pd.concat([signals_df, pd.DataFrame([{'Date': open_prices.index[i], 'Price': open_prices.iloc[i], 'Signal': 'Sell', 'Closed': 'No'}])], ignore_index=True)
                current_position = 'short'

    if current_position != 'none':
        closed_signals.append(('Closed', open_prices.iloc[-1], current_position))
        signals_df = pd.concat([signals_df, pd.DataFrame([{'Date': open_prices.index[-1], 'Price': open_prices.iloc[-1], 'Signal': 'Closed', 'Closed': 'Yes'}])], ignore_index=True)

    print('Closed signals:', closed_signals)  # Print closed signals explicitly
    return orders, closed_signals, current_position, signals_df
...
signals_df = pd.DataFrame(columns=['Date', 'Price', 'Signal', 'Closed'])

# Execute buy/sell orders based on predictions and open prices
orders, closed_signals, current_position, signals_df = execute_orders(predictions_tesla, new_tesla_data['Open'], current_position, signals_df)
...
if current_position != 'none':
    ...
    signals_df = pd.concat([signals_df, pd.DataFrame([{'Date': new_tesla_data.index[-1], 'Price': new_tesla_data['Close'].iloc[-1], 'Signal': 'Closed', 'Closed': 'Yes'}])], ignore_index=True)
