...
def execute_orders(predictions, open_prices):
    orders = []

    for i in range(len(predictions)):
        if predictions[i] == 1:  # Buy signal
            orders.append(('Buy', open_prices[i]))
        elif predictions[i] == -1:  # Sell signal
            orders.append(('Sell', open_prices[i]))
...
orders = execute_orders(new_tesla_predictions, new_tesla_data['Open'])
